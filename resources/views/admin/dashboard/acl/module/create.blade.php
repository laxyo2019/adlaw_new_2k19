...
<div class="row">
	<div class="col-md-12 col-xs-12 col-sm-12 m-auto">
		<div class="box box-primary">
			...
			<div class="box-body">
				<form action="{{route('acl_module.store')}}" method="post"> 
					@csrf
					<div class="row">
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">
							<label class="required">Module Name</label>
							<input type="text" name="name" class="form-control" placeholder="Enter module name" value="{{old('name')}}">
							@error('name')
					...
					<div class="row"  id="dateDiv" style="display: none">
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">		
							...
							<input type="text" name="from" class="form-control datepicker" placeholder="Enter from date" value="{{old('from')}}" readonly> 
							@error('from')
								<span class="invalid-feedback text-danger" role="alert">
								<strong>{{ $message }}</strong>
								</span>
						...
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">
							...
							<input type="text" name="to" class="form-control datepicker" placeholder="Enter to date" value="{{old('to')}}" readonly> 
							@error('to')
								<span class="invalid-feedback text-danger" role="alert">
								<strong>{{ $message }}</strong>
								</span>
					...
					<div class="row">
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">							
							...
							<input type="text" name="link" class="form-control" placeholder="Enter module link" value="{{old('link')}}"> 
							@error('link')
								<span class="invalid-feedback text-danger" role="alert">
								<strong>{{ $message }}</strong>
								</span>
						...
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">
							...
							<input type="text" name="icon" class="form-control" placeholder="Enter icon name" value="{{old('icon')}}">
							@error('icon')
								<span class="invalid-feedback text-danger" role="alert">
								<strong>{{ $message }}</strong>
								</span>
						...
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">
							...
							<select name="module_type" class="form-control" required="required">
								...
								@foreach(MODULETYPE as $key => $value)
									<option value="{{$key}}" {{old('module_type') == $key ? 'selected' : ''}}>{{$value}}</option>
								@endforeach
							</select>
						</div>
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">
							<label class="required">Module Line</label>
							<input type="text" name="line" class="form-control" placeholder="Enter module line" value="{{old('line')}}"  oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1');">
							@error('line')
								<span class="invalid-feedback text-danger" role="alert">
								<strong>{{ $message }}</strong>
								</span>
