...
<div class="row">
	<div class="col-md-12 col-xs-12 col-sm-12 m-auto">
		<div class="box box-primary">
			...
			<div class="box-body">
				<form action="{{route('acl_package.store')}}" method="post"> 
					@csrf
					<div class="row">
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">
							<label class="required">Package Name</label>
							<input type="text" name="name" class="form-control" placeholder="Enter package name" value="{{old('name')}}">
							@error('name')
						...
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">
							...
							<input type="text" name="price" class="form-control" placeholder="Enter package price" value="{{old('price')}}">
							@error('price')
								<span class="invalid-feedback text-danger" role="alert">
								<strong>{{ $message }}</strong>
								</span>
					...
					<div class="row">
						...
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">
							...
							<input type="text" name="duration" class="form-control" placeholder="Enter duration" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value="{{old('price')}}">
							@error('duration')
								<span class="invalid-feedback text-danger" role="alert">
								<strong>{{ $message }}</strong>
								</span>
						...
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">
							...
							<select name="module_id[]" class="form-control select2" multiple="multiple">
								...
								@foreach($modules as $module)
									<option value="{{$module->id}}"  {{ (collect(old('module_id'))->contains($module->id)) ? 'selected':'' }}>{{$module->name}}</option>
								@endforeach
							</select>
							@error('module_id')
								<span class="invalid-feedback text-danger" role="alert">
					...
					<div class="row">
						<div class="col-md-12 form-group">
							...
							<textarea name="description" rows="10" cols="50" class="form-control tinymce" placeholder="" >{{old('description')}}</textarea>
						</div>
					</div>
					<div class="row">
						<div class="col-md-6 col-lg-6 col-sm-12 col-xs-12 form-group">
